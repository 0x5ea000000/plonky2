...
pub fn fri_proof_with_gpu<F: RichField + Extendable<D>, C: GenericConfig<D, F = F>, const D: usize>(
    initial_merkle_trees: &[&MerkleTree<F, C::Hasher>],
    // Coefficients of the polynomial on which the LDT is performed. Only the first `1/rate` coefficients are non-zero.
    lde_polynomial_coeffs: PolynomialCoeffs<F::Extension>,
    // Evaluation of the polynomial on the large domain.
    ...
    let query_round_proofs = timed!(
        ...
        fri_prover_query_rounds_with_gpu::<F, C, D>(initial_merkle_trees, &trees, challenger, n, fri_params, ctx)
    );
...
fn fri_prover_query_rounds_with_gpu<
    F: RichField + Extendable<D>,
    C: GenericConfig<D, F = F>,
    const D: usize,
>(
    ...
    challs.into_par_iter().zip(proofs_vec)
        .map(|(rand, initial_proof)| {
            let x_index = rand.to_canonical_u64() as usize % n;
            fri_prover_query_round_with_gpu::<F, C, D>(initial_proof, trees, x_index, fri_params)
        })
        .collect()
}
...
fn fri_prover_query_round_with_gpu<
    F: RichField + Extendable<D>,
    C: GenericConfig<D, F = F>,
    const D: usize,
>(
